...
The above is rather hard to use, if `elements` is not initialized. Therefore, we can use `@:default` to also define a default value:
...
Also it should be noted that like `@:generic`, partial implementations will cause generation of lots of code.
...
```
class OrderedHash<T> implements tink.Lang {
...
If the `@:forward`-tag has no arguments, then all fields are matched. Otherwise all fields matching either argument are matched.su
...
Tink supports a multitude of notations for short lambdas. Generally, two are different kinds of functions are distinguished: those that return values and those that don't. We'll be calling them functions and procedures respectively (as is the case in Pascal).
...
This syntax is intended for functions with particularly *simple* bodies, the main motivation being to allow writing functions inline with minimal noise, which by its nature is not compatible with complex bodies. In that case, please declare a named (nested) function instead, and use its name.  
...
You can use `@:explain` at class, field and expression level. The first is equivalent to prefixing any field with `@:explain`. The latter two will cause a compiler warning that the position, that will show you the resulting code.
...
You can also log arbitrary expressions by including them in an array literal. Please note that if these are subexpressions of the logged expression, they will be evaluated twice none the less, therefore any side effects will occur twice. You should no use logging to trigger additional side effects.
...
You can prefix a function with @:log. The effect is that each call will be logged, together with parameters and return value (if any). Works for both function expressions and methods/constructors.

## Assertions

To make assertions, you can write `@:assert(...) value`.

Within the assertions, `_` will be substituted for the value.
...
Note that measuring itself can impact the performance. It is advisable to use a cheap implementation of haxe.Log.trace, which is best achieved through batching output.
...
At expression level, tink will use `@:development` metadata for development tools and `@keyword` metadata for pseudo-keywords.