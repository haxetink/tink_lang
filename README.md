...
Tink allows for arbitrary default arguments. Note that the expression will be evaluated for every function call. The generated code is similar to that generated by Haxe. Example:
...
- To have a mandatory option, you can use `_`. 
...
As we see, not specifying an initialization will cause generation of a trigger. If you do specify an initialization, you might just as well use normal property notation. This syntax however allows for a consistent notation in both cases.
...
To translate for loops to while loops, the Haxe compiler does a bit of optimization, because iterators are not particularly fast. In Haxe, currently these rules are hard coded. In tink, there are 3 cases (assuming a single loop target for now):
...
3. The target is a call to a field, e.g. `target.someField()`, where `someField` has metadata `@:tink_for(init, condition, update)`, then we generate the loop as `{ init; while(condition) update; }`. If the structure does not apply, or if the field has no `@:tink_for` metadata, we try the following.
...
Platform|&nbsp;|array|list|string map|string map keys|int map|int map keys
---:|---:|---:|---:|---:|---:|---:|---:
...
&nbsp;|plain|44.07|64.63|110.51|103.06|108.67|102.70
&nbsp;|speedup|4.13|4.34|7.20|5.97|7.14|5.89
nodejs|tink|0.15|0.74|15.89|16.10|11.30|11.19
&nbsp;|plain|0.14|2.10|32.69|23.09|26.30|24.00
&nbsp;|speedup|0.93|2.83|2.05|1.43|2.32|2.14
Chrome|tink|0.16|0.61|18.79|18.70|12.50|12.50
&nbsp;|plain|0.16|1.87|68.70|48.39|28.10|26.60
&nbsp;|speedup|1.00|3.03|3.65|2.58|2.24|2.12
Firefox|tink|0.13|0.44|11.50|4.49|11.10|11.29
&nbsp;|plain|0.12|0.58|24.30|9.50|15.00|14.80
&nbsp;|speedup|0.92|1.31|2.11|2.11|1.35|1.30
...
Bare in mind that "global" is still restricted to classes that actually implement `tink.Lang`. If you're distributing a library based on `tink_lang` it's advisable to test it with and without `force_tink_loops`.

Future version of Haxe should support full inlining of iterators, thereby rendering this optimization obsolete. Depending on the short-term development of the language this feature will be fleshed out.
...
Which reads as "whenever the button is pressed, until it is released the next time, every second trace tick". A bit clunky, but it does get the point across.
